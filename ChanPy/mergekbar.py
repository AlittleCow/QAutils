...
class ChanMergeKbarDirection:
    ...
    def _get_line_direction(self) -> Direction:
        """Get direction from line context"""
        if not self.context:
            return Direction.UNKNOWN
        
        try:
            # Get the global line from context
            ...
            # Fallback to latest line
            latest_line = self.context.get_latest_line()
            if latest_line:
                from .line import LineDirection
                if latest_line.direction == LineDirection.UP:
                    return Direction.UP
                elif latest_line.direction == LineDirection.DOWN:
                    return Direction.DOWN
                    
        except Exception as e:
            self.logger.debug(f"Error getting line direction: {e}")
        
        return Direction.UNKNOWN
    
    def _get_pen_direction(self) -> Direction:
        """Get direction from pen context using latest two pens"""
        if not self.context:
            return Direction.UNKNOWN
        
        try:
            # Get current pens from context
            current_state = self.context.get_current_state()
            if not current_state or len(current_state.current_pens) < 2:
                return Direction.UNKNOWN
            
            # Use the latest two pens to determine direction
            latest_pens = current_state.current_pens[-2:]
            pen1, pen2 = latest_pens
            
            # Import here to avoid circular import
            from .pen import PenDirection
            
            # Determine overall direction from pen sequence
            if pen1.direction == PenDirection.UP and pen2.direction == PenDirection.DOWN:
                # Up pen followed by down pen - overall trend might be up
                if pen2.end_price > pen1.start_price:
                    return Direction.UP
                else:
                    return Direction.DOWN
            elif pen1.direction == PenDirection.DOWN and pen2.direction == PenDirection.UP:
                # Down pen followed by up pen - overall trend might be down
                if pen2.end_price < pen1.start_price:
                    return Direction.DOWN
                else:
                    return Direction.UP
            ...
            self.logger.debug(f"Error getting pen direction: {e}")
