...
/**
 * @brief Structure to represent K-bar data for KT platform
 */
struct KTBarData {
    std::string symbol;
...
/**
 * @brief Structure to represent API call request for KT platform
 */
struct KTApiRequest {
...
// ZeroMQ client class for server communication
class KTZmqClient {
private:
...
public:
     ...
     */
    KTZmqClient(const std::string& server_host = "localhost", int server_port = 5555);
     ...
     * 
     * This method can be called by KT to proactively check if the server
     ...
     * @return json Server response
     */
    json testSingleKBar(const KTBarData& kbar_data);
     ...
     * @return json Server response
     */
    json testSingleKBar(const KTBarData& kbar_data, const std::string& period);
     ...
     * @return json Server response
     */
    json SendKBarSeries(const std::string& symbol, const int period, const std::vector<KTBarData>& kbar_series);
     ...
     * @return json Server response
     */
    json testApiCalls(const std::vector<KTApiRequest>& requests);
    ...
    /**
     * @brief Calculate Simple Moving Average (SMA) for given price data
     * 
...
private:
     ...
     */
    json kbarToJson(const KTBarData& kbar);
     ...
     * @return json JSON representation
     */
    json apiRequestToJson(const KTApiRequest& request);
};
